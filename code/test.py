...
controls = ['MARL', 'conventional']

for control in controls:
    for city in list(location.keys()):
        for time in list(season.keys()):
            ...
            #       Test loop       #
            #########################
            control_mode = control # MARL or conventional
            ...
            print(f'Start: mode {control_mode}, location: {city}, season: {time}')
            ...
            for steps in range(simulation_time * 24):
                ...
                x, S, level, T, CO2, O2, pH, miu, CO2_cons = bio.solve()
                # print(f'substrate: {S}')
                if steps % 24 == 0:
                    day += 1
            ...
            cols = 'x,S,I_sunlight,I_red_light,I_blue_light,level,Fs,CO2_ratio,Facid,Fbase,T,CO2,O2,pH,T_env,RH,wind_speed,Q_heat,miu,miu_light,miu_carbon,miu_pH,miu_S,miu_T,CO2_cons'    
